...
class DataLoader:
    ...
    def __init__(self, data_directory, days, batch_size, drop_last_batch=False):
        self.data_directory = data_directory
        self.days = days
        self.batch_size = batch_size

        total_file = os.path.join(data_directory, "day_day_count.npz")
    ...
    def __iter__(self):
        return iter(_batch_generator(self.data_directory, self.days,
...
def _batch_generator(data_directory, days, batch_size, drop_last_batch):
    previous_file = None
    for day in days:
        filepath = os.path.join(data_directory, "day_{}_reordered.npz".format(day))
        ...
        while batch_start_idx < samples_in_file - batch_size:
            ...
            if previous_file is not None:
                x_int_batch = np.concatenate([previous_file['x_int'], x_int_batch], axis=0)
                x_cat_batch = np.concatenate([previous_file['x_cat'], x_cat_batch], axis=0)
...
def _test():
    generator = batch_generator(data_directory='/data', days=range(23), batch_size=2048)
    t1 = time.time()
    for x_int, lS_o, x_cat, y in generator:
        t2 = time.time()
        time_diff = t2 - t1
        t1 = t2
        print('time: {} x_int shape: {} lS_o.shape: {} x_cat.shape: {} y.shape: {} '.format(
